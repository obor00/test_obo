...
def main():
    devices = get_nvme_emulation_devices()
    check_nvme_emulation_devices(devices, count=4)

    success = True
    for dev in devices:
        name = dev["DevicePath"]
        try:
            print(f"Running nvme emul read/write on {name}")
            result = run_fio(name)
            performance_is_acceptable(result["jobs"][0], name)
